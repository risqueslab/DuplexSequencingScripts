...
Modified Antiref CountMuts.1.41.py compiled by Dana Nachmanson November 8, 2016
Essentially the same as Antiref version by J. Salk 1/16 that was made with the 1.3 version of count-muts.
As J. Salk notated in previous version: same as 1.41 version but the output muts.txt file set to flip the bases sequenced and the types of mutations to the complementary base
This should be used for reverse mapping SSCS data
This allows the outputted data from reverse mapping reads to merged with the normal CountMuts.1.41.py output data for forward mapping reads for combined strand-corrected analsys.


Usage:
cat seq.pileup | CountMuts.1.41_antiref.py [-h] [-d MINDEPTH] [-C MAX_CLONALITY] [-c MIN_CLONALITY] [-n N_CUTOFF] [-s START] [-e END] [-u] > outfile.countmuts
optional arguments:
...
#count and remove deletions
    ...
    print("")
    print("*** NOTE REVERSED OUTPUT FOR REVERSE MAPPING STRANDS ***")
    ...
    print(("A to T:\t%s" % AtoT) + ('\t%.2e\t%.2e\t%.2e' % Wilson(TtoA,  max(Tseq, 1))), file = fOut) #Output is in the form: Mutation type, number of times mutation is oberseved, frequency, 95% positive CI, 95% negative CI (Confidence Intervals are based on the Wilson Confidence Interval)
    print(("A to C:\t%s" % AtoC) + ('\t%.2e\t%.2e\t%.2e' % Wilson(TtoG,  max(Tseq, 1))), file = fOut)
    print(("A to G:\t%s" % AtoG) + ('\t%.2e\t%.2e\t%.2e' % Wilson(TtoC,  max(Tseq, 1))), file = fOut)
    print("\nT's sequenced: %s" % Tseq, file = fOut)
    print("Mutation type\t#\tFrequency\t95% positive CI\t95% negative CI", file = fOut)
    print(("T to A:\t%s" % TtoA) + ('\t%.2e\t%.2e\t%.2e' % Wilson(AtoT,   max(Aseq, 1))), file = fOut)
    print(("T to C:\t%s" % TtoC) + ('\t%.2e\t%.2e\t%.2e' % Wilson(AtoG,  max(Aseq, 1))), file = fOut)
    print(("T to G:\t%s" % TtoG) + ('\t%.2e\t%.2e\t%.2e' % Wilson(AtoC,   max(Aseq, 1))), file = fOut)
    print("\nC's sequenced: %s" % Cseq, file = fOut)
    print("Mutation type\t#\tFrequency\t95% positive CI\t95% negative CI", file = fOut)
    print(("C to A:\t%s" % CtoA) + ('\t%.2e\t%.2e\t%.2e' % Wilson(GtoT,  max(Gseq, 1))), file = fOut)
    print(("C to T:\t%s" % CtoT) + ('\t%.2e\t%.2e\t%.2e' % Wilson(GtoA,   max(Gseq, 1))), file = fOut)
    print(("C to G:\t%s" % CtoG) + ('\t%.2e\t%.2e\t%.2e' % Wilson(GtoC,   max(Gseq, 1))), file = fOut)
    print("\nG's sequenced: %s" % Gseq, file = fOut)
    print("Mutation type\t#\tFrequency\t95% positive CI\t95% negative CI", file = fOut)
    print(("G to A:\t%s" % GtoA) + ('\t%.2e\t%.2e\t%.2e' % Wilson(CtoT,   max(Cseq, 1))), file = fOut)
    print(("G to T:\t%s" % GtoT) + ('\t%.2e\t%.2e\t%.2e' % Wilson(CtoA,   max(Cseq, 1))), file = fOut)
    print(("G to C:\t%s" % GtoC) + ('\t%.2e\t%.2e\t%.2e' % Wilson(CtoG,   max(Cseq, 1))), file = fOut)
