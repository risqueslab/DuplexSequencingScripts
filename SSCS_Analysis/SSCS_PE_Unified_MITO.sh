...
do	
	...
	mkdir LightStrand
	mkdir HeavyStrand
	mv ${elmt}_mem.sscs.mapped.fwd* ./LightStrand
	mv ${elmt}_mem.sscs.mapped.rev* ./HeavyStrand
    ...
    cat ${elmt}.sscs.clipped.no_overlap.pileup | python $DS_PATH/Programs/mut-position.1.31.py -C 1 -d 1 > ${elmt}.SSCS.all.LightStrand.mutpos

	# Filter out all positions that are not mito
	awk '{ if ($1 == "MT2" ) print }' ${elmt}.SSCS.all.LightStrand.mutpos > ${elmt}.SSCS.LightStrand.mutpos
    ...
    cat ${elmt}.sscs.clipped.no_overlap.onlyMITO.pileup | python $DS_PATH/Programs/mut-position.1.31.py -d 1 -C 1 > ${elmt}.SSCS.LightStrand.mutpos
    
	# Generate count_muts file. -C is 1 (turn off clonality cutoff). No -u (keep all, not just uniques). No minimum amount of mutations.
    cat ${elmt}.sscs.clipped.no_overlap.onlyMITO.pileup | python $DS_PATH/Programs/CountMuts.1.41.py -d 1 -C 1 > ${elmt}.SSCS.LightStrand.countmuts
    
    # Mito mutpos post processing
	python $DS_PATH/Programs/Mito/MutationConsequences2.py $TRANS_TABLE $REGION_BED $MITO_REF ${elmt}.SSCS.LightStrand.mutpos ${elmt} 0 1
	python $DS_PATH/Programs/Mito/mutpos_analyzer.py ${elmt}.SSCS.LightStrand.mutpos ${elmt}.consequences.txt ${elmt} $MITO_REFERENCE
    ...
    cat ${elmt}.sscs.clipped.no_overlap.pileup | python $DS_PATH/Programs/SSCS_Analysis/mut-position.1.31_antiref.py -d 1 -C 1 > ${elmt}.SSCS.all.HeavyStrand.mutpos

	# Filter out all positions that are not mito
	awk '{ if ($1 == "MT2" ) print }' ${elmt}.SSCS.all.HeavyStrand.mutpos > ${elmt}.SSCS.HeavyStrand.mutpos
    ...
    cat ${elmt}.sscs.clipped.no_overlap.onlyMITO.pileup | $DS_PATH/Programs/SSCS_Analysis/mut-position.1.31_antiref.py -d 1 -C 1 > ${elmt}.SSCS.HeavyStrand.mutpos
    ...
    cat ${elmt}.sscs.clipped.no_overlap.onlyMITO.pileup | python $DS_PATH/Programs/SSCS_Analysis/CountMuts.1.41_antiref.py -d 1 -C 1 > ${elmt}.SSCS.HeavyStrand.countmuts
    
    # Mito mutpos post processing
	python $DS_PATH/Programs/Mito/MutationConsequences2.py $TRANS_TABLE $REGION_BED $MITO_REF ${elmt}.SSCS.HeavyStrand.mutpos ${elmt} 0 1
	python $DS_PATH/Programs/Mito/mutpos_analyzer.py ${elmt}.SSCS.HeavyStrand.mutpos ${elmt}.consequences.txt ${elmt} $MITO_REFERENCE
