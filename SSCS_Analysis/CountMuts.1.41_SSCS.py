...
Updated 9/12/16 by Dana Nachmanson to account for mutations that arise in reference and anti-reference strands. These mutations are distinguishable in SSCS data, but indistinguishable
in DCS data. Usage is the same as CountMuts.py however, note that when using the unique flag [-u], that the number of 'unique' mutations will be higher than with the previous version.
This is because previously, an "A->T" mutation on the reference strand and a "T->A" mutation on the anti-reference strand were counted as equal and an occurence of both would be counted 
as one mutation. However, now they are treated as independent mutations and an instance of both mutations at a certain position will not be counted as 2 mutations and not 1.


Usage:
cat seq.pileup | CountMuts.1.41_SSCS.py [-h] [-d MINDEPTH] [-C MAX_CLONALITY] [-c MIN_CLONALITY] [-n N_CUTOFF] [-s START] [-e END] [-u] > outfile.countmuts
optional arguments:
...
def CountMutations(o, f, fOut):
    ...
    #DN: Removed conversion of lowercase to uppercase in all nucleotides but n's. 
...
#count and remove deletions
    ...
    #skip sites that fall outside of specified start/end ranges, that have insufficient depth or that have clonal mutations or excess frequency of N's:
          if o.end !=0 and int(linebins[1]) < o.start:
                ...
                pass
          elif (float(max(linebins[4].count('T'),linebins[4].count('t'),linebins[4].count('C'),linebins[4].count('c'),linebins[4].count('G'),linebins[4].count('g'),linebins[4].count('A'),linebins[4].count('a'), (max(newIns.count(n) for n in list(set(newIns))) if newIns != [] else 0), (max(newDels.count(m) for m in list(set(newDels))) if newDels != [] else 0))) / float(depth)) > o.max_clonality:
                pass
          elif (float(max(linebins[4].count('T'),linebins[4].count('t'),linebins[4].count('C'),linebins[4].count('c'),linebins[4].count('G'),linebins[4].count('g'),linebins[4].count('A'),linebins[4].count('a'), (max(newIns.count(n) for n in list(set(newIns))) if newIns != [] else 0), (max(newDels.count(m) for m in list(set(newDels))) if newDels != [] else 0))) / float(depth)) < o.min_clonality:
                pass
          else:
              #remove N entries
    ...
    #count point mutations
                ...
                if linebins[2] == 'A':
                      ...
                      if linebins[4].count('t') > 0: TtoA += (1 if o.unique else linebins[4].count('t'))
                      if linebins[4].count('C') > 0: AtoC += (1 if o.unique else linebins[4].count('C'))
                      if linebins[4].count('c') > 0: TtoG += (1 if o.unique else linebins[4].count('c'))
                      if linebins[4].count('G') > 0: AtoG += (1 if o.unique else linebins[4].count('G'))
                      if linebins[4].count('g') > 0: TtoC += (1 if o.unique else linebins[4].count('g'))

                elif linebins[2] == 'T':
                      Tseq += depth
                      if linebins[4].count('A') > 0: TtoA += (1 if o.unique else linebins[4].count('A'))
                      if linebins[4].count('a') > 0: AtoT += (1 if o.unique else linebins[4].count('a'))
                      if linebins[4].count('C') > 0: TtoC += (1 if o.unique else linebins[4].count('C'))
                      if linebins[4].count('c') > 0: AtoG += (1 if o.unique else linebins[4].count('c'))
                      if linebins[4].count('G') > 0: TtoG += (1 if o.unique else linebins[4].count('G'))
                      if linebins[4].count('g') > 0: AtoC += (1 if o.unique else linebins[4].count('g'))
                
                elif linebins[2] == 'C':
                      Cseq += depth
                      if linebins[4].count('A') > 0: CtoA += (1 if o.unique else linebins[4].count('A'))
                      if linebins[4].count('a') > 0: GtoT += (1 if o.unique else linebins[4].count('a'))
                      if linebins[4].count('T') > 0: CtoT += (1 if o.unique else linebins[4].count('T'))
                      if linebins[4].count('t') > 0: GtoA += (1 if o.unique else linebins[4].count('t'))
                      if linebins[4].count('G') > 0: CtoG += (1 if o.unique else linebins[4].count('G'))
                      if linebins[4].count('g') > 0: GtoC += (1 if o.unique else linebins[4].count('g'))
                
                elif linebins[2] == 'G':
                      Gseq += depth
                      if linebins[4].count('A') > 0: GtoA += (1 if o.unique else linebins[4].count('A'))
                      if linebins[4].count('a') > 0: CtoT += (1 if o.unique else linebins[4].count('a'))
                      if linebins[4].count('T') > 0: GtoT += (1 if o.unique else linebins[4].count('T'))
                      if linebins[4].count('t') > 0: CtoA += (1 if o.unique else linebins[4].count('t'))
                      if linebins[4].count('C') > 0: GtoC += (1 if o.unique else linebins[4].count('C'))
                      if linebins[4].count('c') > 0: CtoG += (1 if o.unique else linebins[4].count('c'))
    ...
    print("Strand Orientation accounted for")
    if o.end != 0: 
